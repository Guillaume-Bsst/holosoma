...
from holosoma_retargeting.config_types.retargeters.omniretarget import FootLockConfig, SelfCollisionConfig
...
class InteractionMeshRetargeter:
    ...
    def __init__(
        ...
        self._init_self_collision(self_collision)
    ...
    def _compute_self_collision_constraints(self, frame_idx: int):
        ...
        Assumes mj_forward has already been called (done by _update_jacobians_and_phis_from_q).

        Returns:
            Js: dict mapping ("self", geom_a, geom_b) -> relative Jacobian (1 x nq)
            phis: dict mapping ("self", geom_a, geom_b) -> signed distance
        """
        if not self._self_collision_enabled:
            return {}, {}
    ...
    def _draw_self_collision_geoms(self) -> None:
