...
class ObjectGripForcePotential(RewardTermBase):
    ...
    def _potential(self, env: WholeBodyTrackingManager) -> torch.Tensor:
        ...
        # (num_envs, history, num_bodies, 3) -> summed magnitude over the anchor bodies. Max over the
        # contact history smooths the single-step dropouts the contact solver produces, matching what
        # UndesiredContacts already does with this buffer. Both hands are summed: squeezing a box is
        # two-handed, and the single nearest anchor would miss half the effort.
