...
class MotionLoader:
    ...
    def _load_data_from_motion_npz(self, motion_file: str, device: str) -> tuple[list[str], list[str]]:
        with cached_open(motion_file, "rb") as f, np.load(f) as data:
            ...
            # Reference robot<->support contact (SYMETRIQUE de object_ref_* : meme forme, calculee
            # par add_support_contact.py = poignet -> surface table, witness + distance). Sert au
            # reward support_surface_contact_error_exp (le robot apprend a placer sa main pres de
            # la table, pas a foncer dedans). Frames T (T,) / witness (T,3) LOCAL table.
    ...
    def extend_with_segments(self, segments: dict[str, torch.Tensor], prepend: bool) -> MotionLoader:
        ...
        # support (table) contact reference : meme padding "pas de contact" sur la transition.
...
class MotionCommand(CommandTermBase):
    ...
    ## plomberie). Consommée par l'obs (le robot voit la table) et le reward robot<->table.
    #########################################################################################
    @property
    ...
    def update_physicality_curriculum(self, success_rate: float) -> None:
        ...
        # force mode: success = survie ET tracking objet. Chaque signal est lissé SÉPARÉMENT, puis
        # on prend le min des DEUX EMA — surtout pas le min instantané avant lissage.
        #
        # Pourquoi: `success_rate` est calculé sur les seuls envs qui resettent CE pas-là, soit
        # ~4096/914 ≈ 4.5 envs — une moyenne de Bernoulli sur ~5 échantillons, donc extrêmement
        # bruitée (alors qu'obj_track porte sur ~2000 envs en contact, quasi lisse). Or le min est
        # concave: E[min(X,y)] << min(E[X],y) dès que X est bruité. Mesuré sur le run qsh0cwdo:
        # les deux composantes tenaient ~0.89 pendant que le min-avant-lissage plafonnait à 0.830
        # (prédiction du modèle binomial: 0.836) — 6 points de pénalité gratuite, qui rendaient un
        # seuil à 0.90 inatteignable (il aurait fallu ~0.96 sur chaque composante).
        # Le lissage étant linéaire, l'EMA de chaque signal converge vers sa VRAIE moyenne; le min
        # de deux quantités convergées exprime bien l'intention ("les deux doivent être bons") sans
        # importer le bruit. NB: à alpha=1 il n'y a pas de min, donc pas de biais — c'est pour ça
        # que le warmup franchissait son gate normalement et que le blocage n'apparaissait qu'après.
    ...
    def update_metrics(self):
        ...
        if self.motion.has_object and self._anchor_body_indexes is not None:
            ...
            # precisement ce qui avait rendu les metriques de prise du run precedent
            # ininterpretables (0.2 N plat pendant que la caisse etait manifestement portee).
            #
            # Ces forces sont aussi la grandeur que lit Phi dans ObjectGripForcePotential.
            names = getattr(self._env.simulator, "_object_contact_body_names", [])
            if names:
                fo = self._env.simulator.contact_forces_object  # (N, B, 3), B = wrists+elbows
                mag = torch.norm(fo, dim=-1)  # (N, B)
                total = mag.sum(dim=-1)
                self.metrics["motion/box_force_total"] = total
                for i, bn in enumerate(names):
                    self.metrics[f"motion/box_force_{bn}"] = mag[:, i]
                # Mains seules vs bras : distingue une PRISE (paumes) d'un simple appui sur
                # l'avant-bras, que la somme confondrait.
                hands = [i for i, bn in enumerate(names) if "wrist_yaw" in bn]
                if hands:
                    self.metrics["motion/box_force_hands"] = mag[:, hands].sum(dim=-1)
                # Restreint aux frames ou la reference porte : hors portage la force est ~0 et
                # diluerait la moyenne. C'est le gating qui manquait a la version precedente --
                # mais il reste base sur ref_contact (main pres de la caisse), donc il inclut encore
                # la prise au sol et le depot sur table, ou le poids est repris par le sol/plateau.
                self.metrics["motion/box_force_in_contact"] = torch.where(
                    ref_contact, total, torch.zeros_like(total)
                )
                # Fraction du poids de la caisse effectivement passee par les contacts robot.
                # ~1.0 = le robot porte tout ; <1 = le sol ou la table reprend le reste.
                if self._object_mass is not None:
                    weight = self._object_mass * 9.81
                    self.metrics["motion/object_weight_n"] = weight
                    self.metrics["motion/box_force_over_weight"] = total / weight.clamp_min(1e-3)
